...
for k=2:N %N
    ...
    % Default measurement observation matrix  and measurement covariance
    % matrix

    if strcmp(settings.non_holonomic,'off') && strcmp(settings.speed_aiding,'off') #Task 2
    ...
    elseif strcmp(settings.gnss_outage,'on')
...
%
%     if strcmp(settings.gnss_outage,'off')
%         ind = zeros(1,3); % index vector, describing available measurements, comment this when GPS outage is on, uncomment otherwise
%         % Check if GNSS measurement is available
%         if t(k)==in_data.GNSS.t(ctr_gnss_data)
%             ind(1:3) = 1;
%             % Update GNSS data counter
