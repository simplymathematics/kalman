...
hold on
mask_no_outage = in_data.IMU.t <= 200;
mask_outage = in_data.IMU.t > 200;
h(1)=plot(in_data.GNSS.pos_ned(2,:),in_data.GNSS.pos_ned(1,:),'b.');
h(2)=plot(out_data.x_h(2,mask_no_outage),out_data.x_h(1,mask_no_outage),'r');
h(3)=plot(out_data.x_h(2,mask_outage),out_data.x_h(1,mask_outage),'g');
h(4)=plot(in_data.GNSS.pos_ned(2,1),in_data.GNSS.pos_ned(1,1),'ks');
title('Trajectory')
ylabel('North [m]')
xlabel('East [m]')
legend(h,'GNSS position estimate','GNSS aided INS trajectory before outage', 'GNSS aided INS trajectory after outage','Start point')
axis equal
grid on
...
xlabel('time [s]')
ylabel('Estimated position - GPS position [m]')
legend('x', 'y')
